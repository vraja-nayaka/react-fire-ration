...
const useUser = (isMy?: boolean) => {
  ...
  const userDetailsRef = useFirestore()
    .collection('users')
    .doc(selectedId);
  const { name = '', avatar = DEFAULT_IMAGE_PATH, lastOnlineTime, experience = 0 } = useFirestoreDocData<IProfile>(userDetailsRef);

  const updatedOnliteTime = Date.now();
  ...
  const editProfile = (data: Partial<IProfile>) => {
    userDetailsRef.set({ ...data, userId, lastOnlineTime: updatedOnliteTime }, { merge: true })
  ...
  const addExperience = (diffExp: number) => {
    userDetailsRef.set({ experience: experience + diffExp, lastOnlineTime: updatedOnliteTime }, { merge: true })
  ...
  const saveAvatar = (file: Partial<IProfile>['file']) => {
    nextAvatarRef.put(file)
      .then(() => userDetailsRef.set({ avatar: GET_BASE_IMAGE_PATH(userId, nextAvatarSrc), lastOnlineTime: updatedOnliteTime }, { merge: true })
        .then(() => enqueueSnackbar('Аватар обновлен', { variant: 'success' })))
      .catch((error) => enqueueSnackbar('Произошла ошибка при сохранении: ' + error, { variant: 'error' }));
  };

  return { isAuth, id: selectedId, name, avatar, experience, lastOnlineTime, editProfile, saveAvatar, addExperience };
};
